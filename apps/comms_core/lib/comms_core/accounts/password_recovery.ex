defmodule CommsCore.Accounts.PasswordRecovery do
  ...
  defp request_with_equalized_work(attrs) do
    ...
    tenant =
      case Administration.active_tenant_by_slug(tenant_slug) do
        {:ok, %TenantView{id: tenant_id, status: :active} = tenant}
    ...
    tenant_id = if tenant, do: tenant.id, else: @dummy_tenant_id
    ...
    case {tenant, user, key_result} do
      {%TenantView{}, %User{} = user, {:ok, key}} -> create_request(user, key)
  ...
  def reset(attrs, revoke_audio) when is_map(attrs) and is_function(revoke_audio, 3) do
    ...
    with :ok <- validate_password(new_password),
      ...
      Repo.transaction(fn ->
        ...
        require_active_tenant_or_rollback(user.tenant_id, :invalid_password_recovery_token)
        ...
        %PasswordRecoveryResult{revoked_session_ids: revoked_session_ids}
      end)
      |> transaction_result()
    else
  ...
  defp create_request(user, key) do
    case Repo.transaction(fn ->
           ...
           require_active_tenant_or_rollback(locked_user.tenant_id, :not_found)
  ...
  defp require_active_tenant_or_rollback(tenant_id, reason) do
    unless active_tenant?(tenant_id), do: Repo.rollback(reason)
  end
