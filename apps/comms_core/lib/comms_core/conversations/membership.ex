defmodule CommsCore.Conversations.Membership do
  ...
  def changeset(value, attrs), do: value |> cast(attrs, [:tenant_id, :conversation_id, :user_id, :role, :joined_at, :left_at, :last_read_sequence]) |> validate_required([:tenant_id, :conversation_id, :user_id, :role, :joined_at]) |> unique_constraint([:conversation_id, :user_id])
end
