defmodule CommsCore.AudioCalls.Lifecycle do
  @moduledoc false
  ...
  def start_with_join_authorized(
    ...
    with :ok <- AuthorizationPolicy.authorize(action, subject, %{id: conversation_id}) do
      Repo.transaction(fn ->
        ...
        ensure_active!(call)
        {_participant, credential} = Participants.admit_and_issue!(call, subject, issuer)
  ...
  defdelegate revoke_for_sessions(tenant_id, session_ids, reason), to: Participants

  @doc "Revokes active admissions issued to one device."
  defdelegate revoke_for_device(tenant_id, device_id, reason), to: Participants

  @doc "Revokes every active admission for a tenant user."
  defdelegate revoke_for_user(tenant_id, user_id, reason), to: Participants

  @doc "Revokes one member's active admission to an exact conversation."
  defdelegate revoke_for_membership(tenant_id, conversation_id, user_id, reason),
    to: Participants

  @doc "Revokes every active admission in an exact conversation."
  defdelegate revoke_for_conversation(tenant_id, conversation_id, reason), to: Participants

  @doc "Revokes all active realtime-media admissions for a tenant."
  defdelegate revoke_for_tenant(tenant_id, reason), to: Participants

  @doc "Revokes active admissions for exactly one media kind in a tenant."
  defdelegate revoke_for_tenant_kind(tenant_id, media_kind, reason), to: Participants

  @doc "Revokes every active admission for one call after provider room termination."
  defdelegate revoke_for_call(tenant_id, call_id, reason), to: Participants
  ...
  @doc false
  defdelegate claim_participant_eviction(participant_id, caller),
    to: Participants,
    as: :claim_eviction
