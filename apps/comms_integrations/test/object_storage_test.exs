defmodule CommsIntegrations.ObjectStorageTest do
  ...
  describe "variant presigns" do
    ...
    test "signs the variant content type so the store rejects any other" do
      ...
      # A client that declares a different type fails the signature rather than
      # storing an object that would later be served inline under that type.
    ...
    test "serves a thumbnail inline under its pinned type" do
      ...
      # on ordinary downloads, and is safe only because the type is pinned.
