defmodule CommsWeb.LegalHoldController do
  ...
  def index(conn, params) do
    with {:ok, holds} <- Governance.list_legal_hold_views(params, conn.assigns.current_subject) do
  ...
  def create(conn, params) do
    ...
    with {:ok, result} <- Governance.create_legal_hold_view(params, conn.assigns.current_subject) do
      conn
  ...
  def release(conn, %{"id" => id} = params) do
    with {:ok, hold} <-
           Governance.release_legal_hold_view(id, params, conn.assigns.current_subject) do
