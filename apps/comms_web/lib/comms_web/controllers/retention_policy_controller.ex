defmodule CommsWeb.RetentionPolicyController do
  ...
  def index(conn, params) do
    with {:ok, policies} <-
           Governance.list_retention_policies(params, conn.assigns.current_subject) do
  ...
  def create(conn, params) do
    ...
    with {:ok, result} <- Governance.create_retention_policy(params, conn.assigns.current_subject) do
      conn
  ...
  def update(conn, %{"id" => id} = params) do
    with {:ok, policy} <-
           Governance.update_retention_policy(id, params, conn.assigns.current_subject) do
