defmodule CommsWeb.WebhookDeliveryController do
  ...
  def index(conn, params) do
    with {:ok, deliveries} <-
           Integrations.list_delivery_views(conn.assigns.current_subject, params) do
      json(conn, %{data: Enum.map(deliveries, &IntegrationPresenter.webhook_delivery/1)})
    end
  end

  def replay(conn, %{"id" => id}) do
    with {:ok, delivery} <- Integrations.replay_delivery_view(id, conn.assigns.current_subject) do
      conn
      |> put_status(:accepted)
