defmodule CommsWeb.ModerationController do
  ...
  def index(conn, params) do
    with {:ok, cases} <- Moderation.list_cases(params, conn.assigns.current_subject) do
  ...
  def create(conn, params) do
    ...
    with {:ok, result} <- Moderation.create_case(params, conn.assigns.current_subject) do
      conn
  ...
  def show(conn, %{"id" => id}) do
    with {:ok, result} <- Moderation.get_case(id, conn.assigns.current_subject) do
      json(conn, %{
  ...
  def add_action(conn, %{"case_id" => case_id} = params) do
    with {:ok, result} <- Moderation.add_action(case_id, params, conn.assigns.current_subject) do
      json(conn, %{
        data: Presenter.moderation_case(result.case),
