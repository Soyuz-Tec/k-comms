defmodule CommsWeb.InstantRoomMessageRateLimit do
  @moduledoc false
  ...
  def consume(conversation_id, subject)
    ...
    with tenant_id when is_binary(tenant_id) <- value(subject, :tenant_id),
      ...
      key_digest = DistributedRateLimit.key_digest(:instant_room_message, material)

      limit =
        Application.get_env(:comms_core, :instant_room_message_rate_limit, 30)
        |> max(1)
  ...
  def consume(_conversation_id, _subject), do: {:error, :forbidden}
