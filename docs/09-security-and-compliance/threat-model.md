...
| T-024 | A poisoned or stale service worker serves unsafe code, persists private communication data, bypasses revocation, or disrupts an active draft/call during update | Worker is same-origin and `/app/` scoped; CSP permits only same-origin workers and manifests; worker response is no-store with an explicit scope allowance; the manifest revalidates; only a fixed offline document and content-hashed same-origin static assets are cacheable; authentication, API, socket, message, file, attachment, invitation, call, media, signed-URL, and non-GET requests are network-only; no offline outbox exists; replacement workers wait for a user-selected Reload; OCI revision is injected into registration and verified in the packaged image | Prove cache contents and update behavior against the exact image and public edge, inspect CDN rules for worker revalidation, re-run push click-through, and physically qualify install/update/rollback on supported iOS and Android releases |
| T-025 | A malicious, oversized, cross-tenant, replayed, or stale whiteboard update leaks content, exhausts clients, or remains visible after access revocation | Workspace-human membership authorization, tenant-composite keys, canonical board sequence, content-bound idempotency, 100,000-operation board cap, 200-element/512,000-byte operation cap, 64,000-byte element cap, allow-listed element types, rejected links/custom data/images/embeds, post-commit broadcast, and join/inbound/outbound channel reauthorization | Qualify exact-candidate multi-client fan-out, replay latency, browser memory, membership removal during active editing, content/log redaction, and production storage alerts |
| T-026 | Direct-call signaling, duplicate Presence, malformed SDP, queued ICE, or asynchronous microphone replacement exhausts an edge/browser, negotiates an unauthorized media type, or retains capture after fallback | Per-event reauthorization and target binding; one peer per user/two per call; deterministic signaling states; audio-only 16 KiB SDP; 2 KiB candidates; database-backed per-actor/call/target message and byte quotas across sessions/replicas; 1 MiB WebSocket limits; 32-entry/64 KiB/15-second client queue; 64-candidate ICE queue; terminal server-untracked fallback; generation-bound track ownership and finally cleanup; LiveKit retained as fallback | Re-run exact-image two-browser direct/fallback journeys, PostgreSQL counter load, capture indicators, and the physical UAE-office direct-path qualification |
