...
| FR-COL-001 | Collaborate on a durable conversation-scoped whiteboard. | Must | Workspace-member authorization, bounded Excalidraw scene operations, idempotent canonical sequencing, clear/replay, realtime cursor presence, multi-device reconciliation, and desktop/mobile browser qualification | Implemented application slice; exact-candidate browser, capacity, and production promotion evidence required |
