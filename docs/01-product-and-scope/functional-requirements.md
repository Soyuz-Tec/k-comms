...
| FR-COL-001 | Collaborate on a durable conversation-scoped whiteboard. | Must | Immediate browser-local draft without passive server writes, idempotent room promotion, workspace-member and active instant-room authorization, bounded Excalidraw scene operations, canonical sequencing, clear/replay, realtime cursor presence, guest scope/abuse denial, multi-device reconciliation, and desktop/mobile browser qualification | Workspace and instant-room application slices implemented; exact-candidate browser, capacity, and production promotion evidence required |
