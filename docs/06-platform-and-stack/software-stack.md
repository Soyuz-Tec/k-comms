...
| Identity | Local password authentication, recovery, rotating sessions, device binding, one-time socket tickets, invitations, and scoped service credentials | Authentication, recovery, lifecycle, revocation, and negative-boundary tests | Implemented 0.3.0 identity; ADR-0023 proposes the OIDC/SCIM boundary but provider-backed OIDC, SCIM, MFA, SAML, and passkeys remain unimplemented |
...
| Media plane | LiveKit WebRTC SFU adapter with one unified audio/video call lifecycle, plus an explicitly opted-in STUN-only peer path for one-to-one audio with automatic LiveKit fallback | Unit/browser direct-negotiation and fallback tests, deployment preflight, and [production two-party cellular audio/video qualification](../11-testing-and-quality/external-media-qualification-2026-07-27.md) | Managed-cloud production WSS/ICE/UDP and two-party cellular audio/video qualified; the new direct path still requires exact-release UAE office qualification, while forced TURN/TLS, group/bandwidth capacity, external screen share, privacy, outage, and incident evidence remain pending |
