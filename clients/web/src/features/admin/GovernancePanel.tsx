...
export function GovernancePanel({ api, users, conversations }: { api: ApiClient; users: User[]; conversations: Conversation[] }) {
  ...
  async function togglePolicy(policy: RetentionPolicy) {
    ...
    const reason = window.prompt(`Enter a reason to ${nextStatus === "active" ? "enable" : "disable"} ${policy.name}.`); if (!reason?.trim()) return;
    setBusy(`policy-${policy.id}`);
    try { const updated = await runWithStepUp(() => api.updateRetentionPolicy(policy.id, { status: nextStatus, version: policy.version, reason: reason.trim() })); setPolicies((current) => current.map((value) => value.id === updated.id ? updated : value)); } catch (cause: unknown) { if (!stepUpWasCancelled(cause)) setError(errorText(cause)); } finally { setBusy(null); }
  }

  async function releaseHold(hold: LegalHold) {
    const reason = window.prompt("Reason for releasing this legal hold"); if (!reason?.trim()) return;
    setBusy(`hold-${hold.id}`); try { const updated = await runWithStepUp(() => api.releaseLegalHold(hold.id, hold.version, reason.trim())); setHolds((current) => current.map((value) => value.id === updated.id ? updated : value)); } catch (cause: unknown) { if (!stepUpWasCancelled(cause)) setError(errorText(cause)); } finally { setBusy(null); }
  }
