...
export function GovernancePanel({ api, users, conversations }: { api: ApiClient; users: User[]; conversations: Conversation[] }) {
  ...
  const duplicateSelectableConversationNames =
    duplicateDirectConversationNames(selectableConversations);
  ...
  return <>
    {error && <div className="inline-notice error" role="alert">{error}<button type="button" aria-label="Dismiss governance error" onClick={() => setError(null)}>×</button></div>}
