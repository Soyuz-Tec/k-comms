...
export function OpsPage() {
  ...
  return <main className="page-shell ops-page" id="main-content">
    ...
    <section className="admin-stats ops-summary" aria-label="Platform operations summary" tabIndex={0}><article><span>Conditions to review</span><strong>{snapshot ? actionableTriage : "—"}</strong><small>{snapshot ? `${triage.length - actionableTriage} healthy checks` : "Awaiting current evidence"}</small></article><article><span>Outbox pending</span><strong>{snapshot?.outbox.pending ?? "—"}</strong><small>{snapshot?.outbox.published ?? 0} published</small></article><article><span>Delivery failures</span><strong>{snapshot ? failures : "—"}</strong><small>Notifications, webhooks and scans</small></article></section>
    {snapshot && <>
